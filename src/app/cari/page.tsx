...
const CariPage: React.FC = () => {
  ...
  useEffect(() => {
    // Fetch both data and quote concurrently
    const fetchAllData = async () => {
      await Promise.all([getData(), getRandomQuote()]);
    };

    fetchAllData();
  }, [getData, getRandomQuote]);
    ...
    <div className="min-h-screen flex flex-col items-center gap-4 md:gap-5 lg:gap-8">
      <div className="bg-gradient-to-r from-ppmb-blue-600 to-ppmb-blue-300 px-[30px] md:px-[100px] flex flex-col py-10 gap-3 items-center w-full">
        ...
        <div className="text-white flex flex-col items-center text-center mt-2 md:mt-4">
          ...
          <text className="italic text-ppmb-100 font-light text-sm md:text-[16px]">
            ── {randomQuote.fullname}, {randomQuote.faculty}{" "}
            {randomQuote.batch}
          </text>
        </div>
      </div>
      ...
      {isSearching ? (
        ...
        <>
          <div
            className={`${
              friends.length == 0 ? "hidden" : "grid"
            } w-full grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-3 md:gap-7 lg:gap-6 px-3 md:px-5 lg:px-8`}
          >
          ...
          <div
            className={`${
              friends.length == 0 ? "flex" : "hidden"
            } justify-center items-center italic md:text-2xl text-ppmb-500`}
          >
