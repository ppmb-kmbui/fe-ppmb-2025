...
export const Sidebar: React.FC = () => {
    ...
    return (
        <div className={`${isExpanded ? "block w-[250px] px-4" : "w-[65px] px-[10px]"} pt-4 gap-3 h-screen hidden md:flex bg-white flex-col duration-300 ease-in-out items-center`} id="bar-fixed" ref={ref}>
            ...
            <div className="flex flex-col gap-[8px] w-full h-full justify-between">
                <div className="flex flex-col gap-[8px] w-full h-full">
                    {MAIN_MENU.map((menu, i) => (
                        <div key={i} className={`${pathname == menu.route ? "bg-ppmb-blue-500 text-ppmb-000" : "hover:bg-ppmb-100 text-ppmb-800"} ${!isExpanded && "justify-center"} group flex items-center px-3 py-2 rounded-lg cursor-pointer`} onClick={() => router.push(menu.route)}>
