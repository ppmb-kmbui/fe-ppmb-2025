      ...
      {isSidebarOpen && (
        <div className="md:hidden fixed inset-0 z-40 bg-white flex flex-col items-center p-4 justify-center">
          <div className="flex justify-end w-full">
            <button className="text-4xl p-2" onClick={handleSidebarToggle}>
              &times;
            </button>
          </div>
          <div className="flex flex-col gap-4 w-full">
            <button className="p-2 text-lg" onClick={() => navigateTo("/")}>
              Beranda
            </button>
            {isAuthenticated && (
              <button
                className="p-2 text-lg"
                onClick={() => navigateTo("/cari")}
              >
                Cari
              </button>
            )}
            {isAuthenticated && user.batch == 2024 && (
              <button
                className="p-2 text-lg"
                onClick={() => navigateTo("/networking")}
              >
                Networking
              </button>
            )}
