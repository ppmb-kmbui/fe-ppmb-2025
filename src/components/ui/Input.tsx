...
export const Input: React.FC<InputProps> = ({
    ...
    return (
        <div className={`${type == "rounded" ? "w-[850px]" : "w-full"} flex flex-col text-lg text-ppmb-800`}>
            ...
            <div className={`${type == "rounded" ? "rounded-3xl px-5 flex-row gap-3 items-center" : "rounded-md px-3"} items-center border-ppmb-blue-600 bg-ppmb-000 border-[2px] py-[5px] flex flex-row font-medium`}>
                {type == "rounded" && <div className="text-xl">{icon}</div>}
                <input className="w-full bg-ppmb-000 focus:outline-none placeholder:font-medium" type={label?.split(" ").includes("Password") && !isVisible ? "password" : "text"} placeholder={placeholder} onChange={(e) => setValue(e.target.value)}/>
